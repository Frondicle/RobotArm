...
mod = pd.read_excel(xlpath, sheet_name="1", header = 0,index_col='dex',usecols=['dex','line','move','espeed','dir','end'])

move = pd.read_excel(xlpath, "1", index_col='dex',usecols=['dex','line','x','y','z','pitch','roll','yaw','fspeed'])
...
def modFrame(frame,ln):
    ...
    dframe = [0x07,0x10,0x00,0x00,0x00,0x04,0x08, move, spd, dir,end]
...
line = 1
while arm.connected:
    while line <= mod.size / 5:
        print ('line: ',line)
        arm.clean_error()
        #if (arm.get_is_moving == False):# and arm.get_tgpio_digital(0) == (1))    
        arm.set_state(state = 0)
...
sender(datas)
mover([305,0,100,-30,0,0,armspeed])#x=move[0], y=move[1], z=move[2], roll=move[4], pitch=move[3], yaw=move[5], speed=move[6]
time.sleep(2)
extruderReset()
